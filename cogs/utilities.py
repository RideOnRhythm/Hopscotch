...
async def get_forecast_async():
  async with aiohttp.ClientSession() as session:
    async with session.get('https://www.pagasa.dost.gov.ph/weather/') as resp:
  ...
  async def dictionary(self, ctx, *, word):
    async with aiohttp.ClientSession() as session:
      async with session.get(
          f'https://api.dictionaryapi.dev/api/v2/entries/en/{word}') as resp:
    ...
    webhook = discord.utils.find(lambda w: w.name == 'Hopscotch Dictionary',
                                 webhooks)
    if not webhook:
      print('creating webhook')
      webhook = await ctx.channel.create_webhook(name='Hopscotch Dictionary')
