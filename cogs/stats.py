...
class Leaderboard(discord.ui.View):
    ...
    @discord.ui.select(cls=discord.ui.Select,
                       options=[
                           ...
                           for label in ('Coins', 'Levels', 'Wins', 'Gems',
                                         'Messages', 'Voice Call Hours',
                                         'Commands')
                       ])
    async def leaderboards(self, interaction: discord.Interaction,
        ...
        embed = await leaderboards(self.cog, tops[select.values[0]])
        await interaction.response.edit_message(embed=embed)


async def leaderboards(self, sort='coins'):
    ...
    if sort in ('coins', 'coin', 'balance', 'bal'):
    ...
    elif sort in ('level', 'lvl', 'lv', 'levels', 'xp', 'exp', 'experience'):
        embed.title += 'level'
        ...
        total_win_list = sorted(total_win_list,
                                key=lambda t: t[1],
                                reverse=True)
...
class Stats(commands.Cog):
    ...
    async def activenow(self, ctx):
        ...
        for member in chit_chatters + eggs:  # s + eggs (real)
            ...
            last_message_channel = self.bot.get_channel(
    ...
    async def members(self, ctx):
        ...
        for member in chit_chatters + eggs:
            ...
            if last_message_unix is not None:
                if time.time(
                ) - last_message_unix > 180 and member.voice is None:
    ...
    async def leaderboard(self, ctx, *, sort='coins'):
        embed = await leaderboards(self, sort)
        await ctx.send(embed=embed, view=Leaderboard(self))
        await database.set_attribute(self.bot.database, ctx.author, 1,
                                     'command_count')
        await database.set_xp(self.bot.database, ctx.author, 1)
