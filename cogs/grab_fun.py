...
class Subject:
    ...
    def apply_action(self, action):
        songs = [
            'Atlantis by Seafret',
            'Sunshine Rainbow White Pony by Wowkie Zhang', 'The N-Word Song',
            'Shinunoga E-Wa', 'Ed Sheeran', 'NGGUP', 'Jepoy D', 'Unhinged',
            'Here I Come'
        ]
    ...
    async def grab(self, ctx):
        if not self.grab_flag:
            ...
            if not self.add_coins:
                rng = 0
            if 'Childe' in self.grab_message and 'Ganyu' in self.grab_message and 'fighting each other' in self.grab_message:
                rng = 366061
            await ctx.send(
                f'{ctx.author.mention} was able to grab {rng} :coin:! ')
